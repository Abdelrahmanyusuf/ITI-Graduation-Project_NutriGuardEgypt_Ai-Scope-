...
`unified_egyptian_rag_database_v2_final.json` as the project-approved
**graduation recipeSource**. Run `npm run demo:prepare` before presenting the project; it
...
The demo always builds its deterministic local retrieval index before it
starts. If `.env` contains a complete Gemini-embedding and Qdrant
configuration, document searches try that remote path first. Network errors,
invalid/expired keys, timeouts, service errors, and empty remote results fall
back to the local index automatically. A short circuit-breaker cooldown avoids
repeating a slow failed call on every question. Nutrition values and comparisons
remain deterministic and are never generated by Gemini. Copy `.env.example` to
the gitignored `.env`; never put real credentials in `.env.example`.

The UI labels recipe data as `verified` for the graduation-project scope while
keeping nutrition values explicitly estimated. This mode is
