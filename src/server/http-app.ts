...
const MealOptionsContextSchema = z.object({ schemaVersion: z.literal("1.0"), lastIntent: z.literal("meal_options"), mealSelectionSessionId: z.string().uuid(), memory: NutritionMemorySchema.optional() }).strict();
const MealSelectionPendingContextSchema = z.object({ schemaVersion: z.literal("1.0"), lastIntent: z.literal("meal_selection_pending"), mealSelectionSessionId: z.string().uuid(), pendingOperationId: z.string().uuid(), memory: NutritionMemorySchema.optional() }).strict();
const MealSelectionAppliedContextSchema = z.object({ schemaVersion: z.literal("1.0"), lastIntent: z.literal("meal_selection_applied"), mealSelectionSessionId: z.string().uuid(), pendingOperationId: z.string().uuid(), memory: NutritionMemorySchema.optional() }).strict();
const ConversationContextSchema = z.discriminatedUnion("lastIntent", [CalorieTargetContextSchema, LighterModificationContextSchema, RecipeReferenceContextSchema, MealPlanContextSchema, MealPlanDraftContextSchema, MealOptionsContextSchema, MealSelectionPendingContextSchema, MealSelectionAppliedContextSchema]);
