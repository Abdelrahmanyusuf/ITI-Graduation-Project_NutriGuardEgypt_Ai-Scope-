...
const agent = runtime?.agent ?? await buildSyntheticDemoAgent(config.nodeEnv as "development" | "test");
...
const server = createNutriGuardHttpServer({
  ...
  pilotConsentReference: runtime ? (config as ReturnType<typeof loadProductionConfig>).pilotConsentReference : "LOCAL-SYNTHETIC-DEMO-CONSENT",
...
server.listen(config.port, host, () => logger.log("info", "server_started", { host, port: config.port, releaseId, mode: runtime ? "production" : "synthetic_demo" }));
