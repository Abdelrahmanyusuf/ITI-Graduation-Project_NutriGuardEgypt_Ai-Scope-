...
function processNutritionData(filePath) {
    ...
    fs.createReadStream(filePath)
        ...
        .on('end', () => {
            ...
            fs.writeFileSync('cleaned_ingredients.json', JSON.stringify(cleanedIngredients, null, 2));
        });
}
...
function processRecipesData(filePath) {
    ...
    fs.createReadStream(filePath)
        ...
        .on('end', () => {
            ...
            fs.writeFileSync('cleaned_recipes.json', JSON.stringify(cleanedRecipes, null, 2));
        });
}

// تشغيل الدوال على ملفاتك
processNutritionData('Egyptian_Food_Categorized.csv');
processRecipesData('Recipes For Eqyption Food.csv');
