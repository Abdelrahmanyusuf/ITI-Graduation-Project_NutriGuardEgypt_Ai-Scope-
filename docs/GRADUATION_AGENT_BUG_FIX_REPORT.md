...
| --- | --- | --- |
| BUG-01 calorie-target meals | One-meal targets return the closest measured serving; daily planning supports 1–10 distinct meals, exact targets, and hard calorie ceilings. A request without a target creates a bounded draft and completes naturally when the user supplies the target in a later turn. | The Agent reports `no_result` when recorded meals cannot satisfy every rule; maximum-calorie plans never silently exceed the ceiling. |
...
| BUG-09 short-term memory | Every intent uses the same `/api/v1/chat` workflow and a shared bounded nutrition memory. It can retain the active recipe, up to eight recent recipe IDs, a meal-plan draft/ready plan, meal count, target/maximum calories, exclusions, a single-meal target, and a progressive lighter-modification state together. Context survives up to 12 intervening replies and can route naturally back from a recipe to an earlier plan. | Raw chat history is not stored; a new chat reload clears context; forged IDs, excessive arrays, invalid numbers, and unknown fields are rejected at the API boundary. |
| BUG-10 user-rule meal planning | General meal examples apply meal count, target/maximum calories, distinct-recipe selection, and recorded ingredient exclusions, including a dairy group. Colloquial constraints such as `ميكنش فيها منتجات ألبان` exclude yogurt, milk, cheese, butter, cream, ghee, and recorded dairy ice cream. Follow-ups such as `خليهم 5 وجبات`, `زود وجبة`, calorie changes, and new exclusions retain the other active rules. | The Agent explicitly states that ingredient filtering is not an allergy or cross-contamination guarantee. Personalized/medical plans remain refused. |
